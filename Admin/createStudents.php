...
if(isset($_POST['save'])){
    try {
        ...
        $otherName = sanitize_input($conn, $_POST['otherName']);
        $admissionNumber = sanitize_input($conn, $_POST['admissionNumber']);
        $classId = sanitize_input($conn, $_POST['classId']);
        $classArmId = sanitize_input($conn, $_POST['classArmId']);
        
        // Validate required fields
        if(empty($firstName) || empty($lastName) || empty($admissionNumber) || empty($classId) || empty($classArmId)) {
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error checking admission number: " . $stmt->error);
        }
        
        $result = $stmt->get_result();
        if($result->num_rows > 0) {
            throw new Exception("This Admission Number Already Exists!");
        }

        // Generate secure default password - you might want to change this
        ...
        $stmt->bind_param("sssssss", $firstName, $lastName, $otherName, $admissionNumber, $defaultPassword, $classId, $classArmId);
        if($stmt->execute()) {
                ...
                <div class="card-body">
                  <form method="post" action="">
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <input type="text" class="form-control" required name="firstName" value="<?php echo isset($row['firstName']) ? $row['firstName'] : '';?>" id="exampleInputFirstName" placeholder="Enter First Name">
                        </div>
                        <div class="col-xl-6">
                        <label class="form-control-label">Lastname<span class="text-danger ml-2">*</span></label>
                      <input type="text" class="form-control" required name="lastName" value="<?php echo isset($row['lastName']) ? $row['lastName'] : '';?>" id="exampleInputLastName" placeholder="Enter Last Name">
                        </div>
                    </div>
                    <div class="form-group row mb-3">
                        <div class="col-xl-6">
                        <label class="form-control-label">Other Name</label>
                        <input type="text" class="form-control" name="otherName" value="<?php echo isset($row['otherName']) ? $row['otherName'] : '';?>" id="exampleInputOtherName" placeholder="Enter Other Name">
                        </div>
                        <div class="col-xl-6">
                        <label class="form-control-label">Admission Number<span class="text-danger ml-2">*</span></label>
                      <input type="text" class="form-control" required name="admissionNumber" value="<?php echo isset($row['admissionNumber']) ? $row['admissionNumber'] : '';?>" id="exampleInputAdmissionNumber" placeholder="Enter Admission Number">
                        </div>
                    </div>
                    <div class="form-group row mb-3">
                        </div>
